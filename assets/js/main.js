(function ($) {
  ...
  /*---about-bullets container activation---*/
  $('.about-bullets-slider').on('changed.owl.carousel initialized.owl.carousel', function (event) {
    $(event.target).find('.owl-item').removeClass('last').eq(event.item.index + event.page.size - 1).addClass('last')}).owlCarousel({
    autoplay: true,
    loop: true,
    nav: true,
...
$('.counting').each(function() {
  ...
  
    {
      ...
      easing:'linear',
      step: function() {
        $this.text(Math.floor(this.countNum) + String(numSign ? numSign : ""));
      },
      complete: function() {
        $this.text(this.countNum + String(numSign ? numSign : ""));
        //alert('finished');
      }
